...
function Remake(incrementScore) {
  ...
  return (
    <div className="App-remake">
      <div className="explod-modal">{count >= 60 ? <ExplodingModal /> : null}</div>
      <HeaderRemake />
      <div className="container-remake">
        <ScoreWindow count={count} score={score} dragId={dragId} isTotalMatch={isTotalMatch} />
        <Grid
          className="gridKey"
          onDragEnd={() => setCount(count + 1)}
          onDragStart={count <= 20 ? () => setScore(score - 75) : () => setScore(score - 225)} alertMe={alertMe()}
          ...
          {items.map(item => (
            <DragItem key={item.id} id={item.id} onMoveItem={moveItem} >
              <GridItem id={dragId[0]++} >
                ...
                {filterInt(item.id) === dragId[0] ? console.log("yes!!!") : console.log("")}
                {isMatch(true) === true ? decrementTotalMatch() && console.log("Très bien, c'est bon !!!") : console.log("c'est pas bon!!!")}
                {isTotalMatch === 0 ? alert("Vous avec Gagné!!") : null}
              </GridItem>
            </DragItem>
          ))
          }
